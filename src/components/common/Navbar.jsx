			...
			<div className="md:w-11/12 p-2 flex items-center justify-between md:justify-around">
				...
				<ul className="gap-10 hidden md:flex">
					{NavbarLinks.map((link, index) => (
						...
						<ul className={`absolute bg-pure-greys-800 bg-opacity-[95%] text-lg gap-y-5 justify-evenly  top-full flex flex-col pl-6 pr-10  py-4 z-50 right-0 w-2/3 transition-all duration-300 ${ openMenu ? "" : "translate-x-[150%]"}`} ref={ref}>
							{NavbarLinks.map((link, index) => (
								<li key={index}>
									{index === 1 ? (
										<>
				...
				<div className="md:hidden relative flex items-center gap-x-4">
					{token && user && user.accountType === ACCOUNT_TYPE.STUDENT && (
					...
					{
						!token && <i className="ri-menu-3-line text-2xl text-richblack-5" onClick={() => {
